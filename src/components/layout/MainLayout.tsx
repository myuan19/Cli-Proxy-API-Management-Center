...
const sidebarIcons: Record<string, ReactNode> = {
  ...
  proxyServers: <IconGlobe size={18} />,
...
export function MainLayout() {
  ...
  const serverVersion = useAuthStore((state) => state.serverVersion) as string | null;
  ...
  const navItems = [
    ...
    { path: '/proxy-servers', label: t('nav.proxy_servers'), icon: sidebarIcons.proxyServers },
  ...
  const handleVersionCheck = async () => {
    ...
    try {
      ...
      const latest = String(data?.['latest-version'] ?? data?.latest_version ?? data?.latest ?? '');
